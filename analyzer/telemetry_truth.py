...
def get_signal(measured: Any, name: str) -> TelemetrySignal[Any]:
    ...
    return TelemetrySignal(
        value=value if quality == "trusted" else None,
        quality=quality,
...
def build_signal_map(measured: "MeasuredState") -> dict[str, TelemetrySignal[Any]]:
    def trusted(value: Any, *, source: str, confidence: float = 0.85, invalid_reason: str = "") -> TelemetrySignal[Any]:
        if value is None or value == "":
            return TelemetrySignal(
                ...
                confidence=0.0,
                source=source,
                invalid_reason=invalid_reason or "not_extracted",
            )
        return TelemetrySignal(
            value=value,
            quality="trusted",
    ...
    def proxy(value: Any, *, source: str, confidence: float = 0.6) -> TelemetrySignal[Any]:
        if value is None or value == "":
            return TelemetrySignal(
                value=None,
                quality="unknown",
    ...
    return {
        "mean_front_rh_at_speed_mm": trusted(measured.mean_front_rh_at_speed_mm, source="ride_height_channels"),
        "mean_rear_rh_at_speed_mm": trusted(measured.mean_rear_rh_at_speed_mm, source="ride_height_channels"),
        "front_rh_std_mm": trusted(measured.front_rh_std_mm, source="ride_height_channels"),
        "rear_rh_std_mm": trusted(measured.rear_rh_std_mm, source="ride_height_channels"),
        "front_rh_std_hs_mm": trusted(measured.front_rh_std_hs_mm, source="ride_height_channels"),
        "front_shock_oscillation_hz": trusted(measured.front_shock_oscillation_hz, source="shock_velocity_zero_crossing", confidence=0.8),
        "rear_shock_oscillation_hz": trusted(measured.rear_shock_oscillation_hz, source="shock_velocity_zero_crossing", confidence=0.8),
        "front_heave_travel_used_pct": trusted(measured.front_heave_travel_used_pct, source="heave_deflection_channel"),
        "rear_heave_travel_used_pct": trusted(measured.rear_heave_travel_used_pct, source="heave_deflection_channel"),
        "front_braking_lock_ratio_p95": trusted(measured.front_braking_lock_ratio_p95, source="wheel_speed_delta", confidence=0.82),
        "rear_power_slip_ratio_p95": trusted(measured.rear_power_slip_ratio_p95, source="wheel_speed_delta", confidence=0.82),
        "bottoming_event_count_front_clean": trusted(measured.bottoming_event_count_front_clean, source="ride_height_clean_track_filter"),
        "bottoming_event_count_rear_clean": trusted(measured.bottoming_event_count_rear_clean, source="ride_height_clean_track_filter"),
        "bottoming_event_count_front_kerb": trusted(measured.bottoming_event_count_front_kerb, source="ride_height_kerb_filter"),
        "bottoming_event_count_rear_kerb": trusted(measured.bottoming_event_count_rear_kerb, source="ride_height_kerb_filter"),
        "front_pressure_mean_kpa": trusted(measured.front_pressure_mean_kpa, source="tyre_pressure_channel"),
        "rear_pressure_mean_kpa": trusted(measured.rear_pressure_mean_kpa, source="tyre_pressure_channel"),
        "front_carcass_mean_c": trusted(measured.front_carcass_mean_c, source="tyre_temp_channel"),
        "rear_carcass_mean_c": trusted(measured.rear_carcass_mean_c, source="tyre_temp_channel"),
        "splitter_rh_p01_mm": trusted(measured.splitter_rh_p01_mm, source="splitter_ride_height_channel"),
        "pitch_range_braking_deg": trusted(measured.pitch_range_braking_deg, source="pitch_from_ride_height", confidence=0.8),
        "pitch_mean_braking_deg": trusted(measured.pitch_mean_braking_deg, source="pitch_from_ride_height", confidence=0.8),
        "understeer_mean_deg": proxy(measured.understeer_mean_deg, source="steering_yaw_proxy"),
        "understeer_low_speed_deg": proxy(measured.understeer_low_speed_deg, source="steering_yaw_proxy"),
        "understeer_high_speed_deg": proxy(measured.understeer_high_speed_deg, source="steering_yaw_proxy"),
        "body_slip_p95_deg": proxy(measured.body_slip_p95_deg, source="body_velocity_proxy"),
        "yaw_rate_correlation": proxy(measured.yaw_rate_correlation, source="yaw_fit_proxy", confidence=0.7),
        "front_rh_settle_time_ms": TelemetrySignal(
            ...
            confidence=min(0.95, 0.55 + getattr(measured, "front_settle_valid_clean_events", 0) * 0.08) if front_settle_valid else 0.0,
            source="event_based_clean_disturbance_response",
            invalid_reason="" if front_settle_valid else (getattr(measured, "front_settle_invalid_reason", "") or "insufficient_clean_events"),
        ),
        "rear_rh_settle_time_ms": TelemetrySignal(
            value=measured.rear_rh_settle_time_ms if rear_settle_valid else None,
            quality="trusted" if rear_settle_valid else "unknown",
            confidence=min(0.95, 0.55 + getattr(measured, "rear_settle_valid_clean_events", 0) * 0.08) if rear_settle_valid else 0.0,
            source="event_based_clean_disturbance_response",
            invalid_reason="" if rear_settle_valid else (getattr(measured, "rear_settle_invalid_reason", "") or "insufficient_clean_events"),
        ),
    }
