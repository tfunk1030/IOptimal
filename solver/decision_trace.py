...
def _estimate_gain_ms(parameter: str, measured: Any) -> float:
    if parameter in {"front_heave_nmm", "front_heave_index"}:
        return round(
    ...
    if parameter in {"rear_third_nmm", "rear_heave_index"}:
        return round(
...
def _estimate_cost_ms(parameter: str, proposed_value: Any, current_value: Any) -> float:
    try:
        ...
        delta = 0.0
    if parameter in {"front_heave_nmm", "rear_third_nmm", "front_heave_index", "rear_heave_index"}:
...
def _parameter_spec(car_name: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "rationale": "Front heave support is driven by travel margin, clean bottoming, and aero-platform control.",
        },
        {
        ...
        },
        {
            ...
            "rationale": "Rear heave support trades rear platform control against exit traction compliance.",
        },
        {
...
def build_parameter_decisions(
    *,
    ...
    legality_text = "garage validated" if legality is None or legality.valid else "garage validation warning"
    ...
    if car_name.lower() == "ferrari":
        for warning in list(getattr(current_setup, "decode_warnings", []) or [])[:3]:
            decisions.append(
                ParameterDecision(
