...
setup-to-performance mapping, then uses acquisition functions to intelligently
explore the parameter space. This finds optimal setups much faster than grid
search or random sampling because it learns WHERE to look.

The key insight: iRacing's physics model is a black box with unknown nonlinearities.
The physics solver gives us one good starting point, but the ACTUAL lap time
response surface may have local optima that physics-based reasoning misses.
...
How it works:
    ...
    3. Score each sample using the lap time sensitivity model
...
class SimpleGP:
    """Minimal Gaussian Process regressor using RBF kernel.

    This is a lightweight implementation that avoids the sklearn dependency.
    Uses the squared exponential (RBF) kernel with automatic length scale.
    """
...
class BayesianOptimizer:
    """Bayesian optimization of setup parameters."""

    PARAM_SPEC = [
        # (name, garage_attr_lo, garage_attr_hi, step)
        ...
        ("front_arb_blade", "arb_blade", 1),
        ("rear_arb_blade", "arb_blade", 1),
    ]

    def __init__(self, car: CarModel, track: TrackProfile, scorer: callable | None = None):
        self.car = car
        self.track = track
        ...
        self.bounds_lo: list[float] = []
        self.bounds_hi: list[float] = []
        self.steps: list[float] = []

        for name, gr_attr, step in self.PARAM_SPEC:
            rng = getattr(self.gr, gr_attr, (0, 100))
            self.param_names.append(name)
            self.bounds_lo.append(float(rng[0]))
            self.bounds_hi.append(float(rng[1]))
    ...
    def _denormalize(self, X_norm: np.ndarray) -> np.ndarray:
        """Convert from [0, 1] back to parameter space."""
        return X_norm * (self.bounds_hi - self.bounds_lo) + self.bounds_lo

    def _snap_to_garage(self, params: np.ndarray) -> np.ndarray:
        """Snap parameters to garage-legal values."""
        result = params.copy()
    ...
    def _default_scorer(self, params: dict[str, float]) -> float:
        """Default scoring function based on physics heuristics.

        Returns a score where HIGHER is better.
        Combines: aero stability, mechanical grip, balance, toe drag.
        """
        heave = params.get("front_heave_nmm", 50)
        third = params.get("rear_third_nmm", 530)
        ...
        # Aero platform: excursion-based (lower excursion = better)
        ...
        # Simple LLTD estimate from ARB blades (more rear = lower LLTD)
        lltd_est = self.car.weight_dist_front + 0.03 * f_arb - 0.03 * r_arb + 0.05
        balance_score = max(0, 1.0 - abs(lltd_est - optimal_lltd) / 0.10)

        # Camber: peak near -3.5F / -2.5R
        camber_score = (max(0, 1.0 - abs(f_cam + 3.5) / 2.0) * 0.5 +
                        max(0, 1.0 - abs(r_cam + 2.5) / 2.0) * 0.5)
    ...
    def optimize(
        ...
        for i in range(n_initial):
            X[i] = self._snap_to_garage(X[i])

        # Add physics baseline if provided
        if physics_baseline is not None:
            baseline_vec = np.array([physics_baseline.get(n, (lo + hi) / 2)
                                     for n, lo, hi in zip(self.param_names, self.bounds_lo, self.bounds_hi)])
            baseline_vec = self._snap_to_garage(baseline_vec)
            X = np.vstack([baseline_vec.reshape(1, -1), X])

        # Score initial samples
        y = np.array([
            self.scorer({n: X[i, j] for j, n in enumerate(self.param_names)})
            for i in range(len(X))
        ])

        history: list[BayesianCandidate] = []
        baseline_score = y[0] if physics_baseline is not None else np.mean(y)

        # Bayesian optimization loop
        gp = SimpleGP(length_scale=0.3, noise=1e-3)
        ...
        for iteration in range(n_iterations):
            ...
            n_candidates = 500
            X_cand_norm = rng.random((n_candidates, self.dim))
            ...
            next_x = self._snap_to_garage(next_x)

            next_score = self.scorer({n: next_x[j] for j, n in enumerate(self.param_names)})
        ...
        return BayesianResult(
            ...
            physics_baseline_score=float(baseline_score),
