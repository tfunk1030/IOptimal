...
4. Three minor cleanup items: duplicate `import re` (was inside `track_key_from_name` despite module-level import), unused `import math` in `empirical_models.py`, unused `import json` and `SessionDelta` in `ingest.py`.
...
**Fix:** Added the following pairs (with appropriate confidence-encoded directions). Effects merged into existing entries to avoid clobbering:

- **Cross-axle aero coupling** (merged into `front_heave_nmm` and `rear_third_nmm` "+" entries):
  - `front_heave_nmm` "+" → `dynamic_rear_rh_mm` (`~`)
  - `rear_third_nmm` "+" → `dynamic_front_rh_mm` (`~`)
- **Brake bias** (merged): added `front_braking_lock_ratio_p95` (`+`) — more front bias = more front lock risk.
...
- **Fuel load** (`fuel_l` "+"): `dynamic_rear_rh_mm` (`-`), `lltd_measured` (`~`).

The reverse-direction auto-generation block (line ~189) correctly handles all these (verified — it inverts `+`↔`-` and leaves `~` unchanged). Total entries went from 36 to 52 (8 new "+" entries; 8 auto-generated "-" reverses).

**Bug caught during fix:** my first attempt added entries below the existing dict definition for `front_heave_nmm` and `rear_third_nmm`, which silently OVERWROTE the existing rich effect lists with single-element lists. Caught by spot-check (`KNOWN_CAUSALITY[('front_heave_nmm','+')]` had only `[('dynamic_rear_rh_mm','~')]`). Fixed by merging the new entries into the existing definitions in place.
...
### Confidence classification (delta_detector.py:543-553 in current file)
...
This matches CLAUDE.md's "single-change=1.0, two-change=0.5, multi=0" intent. The actual numeric weighting happens later in `_fit_lap_time_sensitivity` (line ~590-616 of empirical_models.py), where:
- `num_changes ≤ 1` → weight 1.0
- `num_changes == 2` → weight 0.5
- `num_changes ≥ 3` → weight `1.0 / num_changes`

The audit-described "multi=0" is a slight overstatement of the implementation — multi-change deltas DO contribute, but with `1/n` discount AND a `confidence_level not in (high, medium)` filter. The combined effect is functionally close to "0" for low-confidence multi-change deltas.

### Min-sessions gate
- Non-prediction corrections: `MIN_SESSIONS_FOR_CORRECTIONS = 5`, used in `_compute_corrections` for roll gradient and LLTD aggregates.
- Prediction-error corrections: `if len(errors) < 3: continue`. Correct — these are diff-based and need fewer samples.
- Roll-gain thermal calibration: `if sample_count < 3`.
...
### Time decay (empirical_models.py:_obs_time_weight)
...
### Experiment gating for sensitivity (empirical_models.py:_fit_lap_time_sensitivity)
Discards deltas where `lap_time_delta < 0.01s` or `> 3.0s` (clearly noise / different session). Requires `confidence_level in (high, medium)`. Single-change weight 1.0, two-change 0.5, multi-change `1/n`. Robust statistics: median-based outlier removal at 3·MAD before computing weighted mean. Shrinkage toward zero with denominator 5.0 (recently increased from 3.0 — see comment in code). All sensible.

### `--all-laps` mode (ingest.py:ingest_all_laps)
...
### Auto-calibration update (ingest.py:_update_auto_calibration)
...
- **Slice #14 (Watcher)**: confirmed in F5 — Windows safety relies on `watcher/service.py` queueing. Verify in that slice that the queue is actually serial.
...
- `python -m pytest tests/ -q` — confirmed 3 pre-existing failures (test_acura_steps_1_3_runnable_4_6_blocked, test_full_report_blocked_steps_includes_cascaded, test_support_tier_mapping). All three reproduce on the unmodified base branch (verified by stashing my changes and re-running). They are NOT caused by this slice — they are pre-existing failures from cross-slice work (calibration semantics + run trace), not learner.
- KNOWN_CAUSALITY spot-check confirms all original entries preserved and 8 new entry families added (16 total with reverse-generated opposites). Entries-by-key count: 52.
...
Pipeline E2E test is therefore documented as "not required" per the worker instructions.
