...
**Fix in this PR:** none — calling
`apply_calibrated_models_to_car(car_name, ...)` from the calibrator
would couple this module to `car_model/auto_calibrate.py` internals and
re-import a `Car` instance just to mutate the flag, with no benefit
inside the CLI. Adding a one-line docstring note is the right minimal
change but lives in `calibrate_dampers.py`. Made the doc-only addition
in this PR (see Fix below).
...
- `python -m pytest tests/ -q --tb=line --ignore=tests/test_webapp_routes.py`
  before fix: 3 failed, 346 passed, 17 skipped, **1 warning**
  (`Auto-calibration failed for cadillac: 'str' object has no
  attribute 'get'`).
- After fix: same pass/fail counts (3 pre-existing failures unrelated
  to this slice), **warning gone**.
- `tests/test_webapp_routes.py` is collection-error due to
  `fastapi` not installed — unrelated to this slice.
- Smoke E2E (`pipeline.produce`) intentionally not re-run because
  data files in this slice are read-mostly and the only changed file
  (`cadillac/models.json`) is a stub the pipeline never reaches
