...
- **Behaviour:** `DamperModel.digressive_exponent: float = 1.0` is fielded with
  the same default. The `getattr` fallback was redundant.
...
- **Behaviour:** When `has_heave_dampers=True` (Ferrari only today), the solver
  ...
  `10/40/5/10/40` — magic numbers wholly uncoupled from car physics.
...
- **Behaviour:** A comment baked the BMW/Sebring observation
  ...
  car-agnostic helper. Misleading for non-BMW callers.
