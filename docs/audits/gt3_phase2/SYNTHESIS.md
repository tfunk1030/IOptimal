...
> **Implementation status (2026-04-27, latest batch W4.3 + W5.3):** Wave 1 (W1.1–W1.3) + Wave 2 (W2.1–W2.4) + Wave 3 (W3.1–W3.3) + Wave 4 (W4.1–W4.3) + W5.2 + W5.3 shipped on `claude/merge-audits-wave1-DDFyg`. All 12 audit PRs merged into the same branch. 15 of 22 work units done (~258 h shipped, ~50%). 665 tests pass, 0 new regressions. GT3 IBT runs end-to-end through pipeline → solver → writer; the engineering report renders 4 corner spring rates instead of "Heave F: 0 N/mm"; analyzer skips phantom heave-bottoming alarms; causal graph routes GT3 sessions through corner-spring nodes. **iRacing schema round-trip validation requires manual driver-side QA** (no offline XSD available). Wave 5 W5.1 (pipeline produce/reason/report GT3 conditional) is the only Wave 5 unit remaining. See [`IMPLEMENTATION_STATUS.md`](IMPLEMENTATION_STATUS.md) for the full progress log.
...
### Wave 5 — pipeline + analyzer (3 units; ~62 h) — **W5.2 + W5.3 done; W5.1 remains**

| # | Title | Files | Effort | Status | Depends on |
|---|---|---|---|---|---|
| W5.1 | Pipeline produce/reason/report GT3 conditional | `pipeline/produce.py` (heave_spring access, JSON output guards, alias dict), `pipeline/reason.py` (heave floor checks), `pipeline/report.py` (display panels) | 24 h | TODO (next critical-path) | W1.1, W1.2, W2.1 |
