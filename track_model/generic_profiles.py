...
_SHOCK_VEL_TEMPLATES: dict[str, dict[str, float]] = {
    "smooth": {
        "p50_front": 0.015,
        "p95_front": 0.080,
        "p99_front": 0.140,
...
# Track classification database (name fragment → roughness, corner style)
_KNOWN_TRACKS: dict[str, tuple[str, str]] = {
    "daytona":      ("smooth",  "high_speed"),
    "le mans":      ("smooth",  "high_speed"),
    "lemans":       ("smooth",  "high_speed"),
...
def _estimate_speed_profile(
    ...
    median_speed = max(120.0, min(260.0, median_speed))

    if track_style == "high_speed":
        pct_above_200 = max(0.10, min(0.60, 0.40 - corner_density * 0.03))
...
def generate_generic_profile(
    ...
    Args:
        name: Track name (e.g., "Silverstone")
        config: Track configuration (e.g., "Arena Grand Prix")
    ...
    for key, (r, s) in _KNOWN_TRACKS.items():
        if key in name_lower:
            roughness_out = r
            track_style = s
            break

    sv = _SHOCK_VEL_TEMPLATES.get(roughness_out, _SHOCK_VEL_TEMPLATES["medium"])
    ...
    return TrackProfile(
        ...
        # Clean track ≈ overall (no kerb data for generic profiles)
