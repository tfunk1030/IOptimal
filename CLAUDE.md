...
- **GT3 Phase 2 — 12 audit PRs merged + Wave 1 + Wave 2 + Wave 3 + Wave 4 + Wave 5.2 + Wave 5.3 shipped (2026-04-27):** All 12 audit PRs (#103–#114, ~329 findings, ~511 h estimated) merged into `claude/merge-audits-wave1-DDFyg` (mirrors `gt3-phase0-foundations`). 15 of 22 work units complete (~258 h shipped, ~253 h remaining ≈ 50% of total). The branch carries the full Phase 2 audit corpus under `docs/audits/gt3_phase2/` plus a [`IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) tracking doc updated after every batch. Suite: **665 passed, 32 skipped, 1 deselected, 0 new regressions** (pre-existing `test_support_tier_mapping` data-dependent failure unchanged). +232 new GT3-specific tests vs the 2026-04-11 baseline of 295.
  ...
  - **Wave 4.3 + Wave 5.3 (DONE; commit `2792f3b`, +1127/-110, 10 files):**
    ...
    - **Combined state after this batch:** GT3 IBT runs end-to-end through pipeline → solver → writer → report cleanly. Engineering report renders 4 corner spring rates (no "Heave F:" garbage). Analyzer no longer fires phantom heave-bottoming alarms; causal graph routes GT3 through corner-spring nodes. `garage_validator` never mutates GT3 step data. **Wave 4 COMPLETE (3/3 units). Wave 5 W5.2 + W5.3 done; W5.1 (pipeline produce/reason/report GT3 conditional) remains.**
  - **Remaining Phase 2 plan (~253 h, 7 of 22 units):** W5.1 (pipeline produce/reason/report GT3 conditional, ~24 h) + W6.1 (objective + sensitivity GT3 guards, ~14 h) is the next batchable pair (~38 h, disjoint files: `pipeline/*.py` vs `solver/{objective,sensitivity,laptime_sensitivity}.py`). After that the critical path is W7.1/W7.2 (auto-calibrate + GarageOutputModel for GT3, ~80 h, gated on varied-spring IBT capture) → W9.x (UI/CLI/tests/docs, ~62 h) → W10.1 (E2E smoke + 7 remaining GT3 cars, ~80 h+) ≈ ~258 h. Wave 6.2/6.3 (learner GT3, ~24 h) and Wave 8 (DB schema migration + watcher GT3, ~43 h) parallelizable. See [`docs/audits/gt3_phase2/SYNTHESIS.md`](docs/audits/gt3_phase2/SYNTHESIS.md) for the full plan and [`docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) for the rolling status log.
