...
- **GT3 Phase 2 — 12 audit PRs merged + Wave 1 + Wave 2 (all 4 units) + Wave 3.1 + W3.2 shipped (2026-04-27):** All 12 audit PRs (#103–#114, ~329 findings, ~511 h estimated) merged into `claude/merge-audits-wave1-DDFyg` (mirrors `gt3-phase0-foundations`). 9 of 22 work units complete (~158 h shipped, ~353 h remaining). The branch carries the full Phase 2 audit corpus under `docs/audits/gt3_phase2/` plus a [`IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) tracking doc updated after every batch. Suite: **573 passed, 32 skipped, 1 deselected, 0 new regressions** (pre-existing `test_support_tier_mapping` data-dependent failure unchanged). +140 new GT3-specific tests vs the 2026-04-11 baseline of 295.
  ...
  - **Wave 2.4 + Wave 3.2 (DONE; commit `aa0beda`, +990/-99, 8 files):**
    ...
    - **Combined state after this batch:** GT3 IBT now runs through Step 1 → Step 6 cleanly. Step 4 ARB output rotates from baseline (size-label live tuning); Porsche 992 RR LLTD targets the empirically-correct 0.45. Damper output respects per-car polarity + range; GT3 dampers written per-axle (no silent L/R loss). **Setup writer still raises** because GT3 PARAM_IDS are empty stubs — that's W4.1 (BMW M4 GT3) and W4.2 (Aston, Porsche 992).
  - **Remaining Phase 2 plan (~353 h, 13 of 22 units):** W4.1 (BMW M4 GT3 setup writer) + W3.3 (fuel constants 89L → per-car cap) is the next batchable pair (~24 h, disjoint files). Then W4.2 (Aston + Porsche 992 PARAM_IDS, ~24 h alone). Critical path after that: W4.3 (output guards) → W7.1/W7.2 (auto-calibrate + GarageOutputModel for GT3) → W9.x (UI/CLI/tests/docs) → W10.1 (E2E smoke + 7 remaining GT3 cars) ≈ ~254 h. See [`docs/audits/gt3_phase2/SYNTHESIS.md`](docs/audits/gt3_phase2/SYNTHESIS.md) for the full plan and [`docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) for the rolling status log.
