...
- **GT3 Phase 2 — 12 audit PRs merged + Wave 1 + Wave 2 + Wave 3 + Wave 4.1 + Wave 4.2 + Wave 5.2 shipped (2026-04-27):** All 12 audit PRs (#103–#114, ~329 findings, ~511 h estimated) merged into `claude/merge-audits-wave1-DDFyg` (mirrors `gt3-phase0-foundations`). 13 of 22 work units complete (~230 h shipped, ~281 h remaining ≈ 45% of total). The branch carries the full Phase 2 audit corpus under `docs/audits/gt3_phase2/` plus a [`IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) tracking doc updated after every batch. Suite: **641 passed, 32 skipped, 1 deselected, 0 new regressions** (pre-existing `test_support_tier_mapping` data-dependent failure unchanged). +208 new GT3-specific tests vs the 2026-04-11 baseline of 295.
  ...
  - **Wave 4.2 + Wave 5.2 (DONE; commit `d124944`, +1307/-24, 9 files):**
    ...
    - **Combined state after this batch:** GT3 IBT writes a valid `.sto` for **all 3 sampled GT3 cars** (BMW M4 GT3, Aston, Porsche 992) with per-car YAML divergences honoured. Analyzer parses GT3 YAML correctly (was silently falling to BMW GTP). `sto_binary` recognises GT3 STO filenames. iRacing schema round-trip still pending (W4.3).
  - **Remaining Phase 2 plan (~281 h, 9 of 22 units):** W4.3 (output guards + iRacing schema round-trip, 14h) + W5.3 (analyzer extract/diagnose GT3 awareness, 14h) is the next batchable pair (~28 h, disjoint files). Closes Wave 4 + Wave 5. Critical path after that: W7.1/W7.2 (auto-calibrate + GarageOutputModel for GT3, ~80 h, gated on varied-spring IBT capture) → W9.x (UI/CLI/tests/docs, ~62 h) → W10.1 (E2E smoke + 7 remaining GT3 cars, ~80 h+) ≈ ~206 h. Wave 6 (learner GT3 awareness, ~56 h) and Wave 8 (DB schema migration + watcher GT3, ~43 h) parallelizable. See [`docs/audits/gt3_phase2/SYNTHESIS.md`](docs/audits/gt3_phase2/SYNTHESIS.md) for the full plan and [`docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) for the rolling status log.
