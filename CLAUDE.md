...
- **GT3 Phase 2 — 12 audit PRs merged + Wave 1 + Wave 2 + Wave 3 + Wave 4 + Wave 5 + Wave 6.1 shipped (2026-04-27):** All 12 audit PRs (#103–#114, ~329 findings, ~511 h estimated) merged into `claude/merge-audits-wave1-DDFyg` (mirrors `gt3-phase0-foundations`). 17 of 22 work units complete (~296 h shipped, ~215 h remaining ≈ 58% of total). The branch carries the full Phase 2 audit corpus under `docs/audits/gt3_phase2/` plus a [`IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) tracking doc updated after every batch. Suite: **703 passed, 32 skipped, 1 deselected, 0 new regressions** (pre-existing `test_support_tier_mapping` data-dependent failure unchanged). +270 new GT3-specific tests vs the 2026-04-11 baseline of 295.
  ...
  - **Remaining Phase 2 plan (~215 h, 5 of 22 units):** W6.2 (Learner KNOWN_CAUSALITY GT3 + STEP_GROUPS, ~6 h) + W6.3 (Learner empirical models + observation schema GT3, ~18 h) is the next batch (~24 h, sequential within Wave 6). Closes Wave 6 fully. Critical path after that: W7.1 (GT3 GarageSetupState + GarageOutputModel, ~24 h, can land with stubbed garage-output models) → W7.2 (auto-calibrate per-car for GT3, ~56 h, gated on varied-spring IBT capture for BMW M4 GT3 at the same track) → W9.x (UI/CLI/tests/docs, ~62 h) → W10.1 (E2E smoke + 7 remaining GT3 cars, ~80 h+) ≈ ~222 h. W8.1+W8.2 (DB schema migration + watcher GT3, ~43 h) parallelizable with all of the above. See [`docs/audits/gt3_phase2/SYNTHESIS.md`](docs/audits/gt3_phase2/SYNTHESIS.md) and [`docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md).
