...
- **GT3 Phase 2 — 12 audit PRs merged + Wave 1 + Wave 2 + Wave 3 + Wave 4 + Wave 5 + Wave 6 shipped (2026-04-27):** All 12 audit PRs (#103–#114, ~329 findings, ~511 h estimated) merged into `claude/merge-audits-wave1-DDFyg` (mirrors `gt3-phase0-foundations`). **19 of 22 work units complete (~320 h shipped, ~191 h remaining ≈ 63% of total).** The branch carries the full Phase 2 audit corpus under `docs/audits/gt3_phase2/` plus a [`IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) tracking doc updated after every batch. Suite: **720 passed, 33 skipped, 1 deselected, 0 new regressions** (pre-existing `test_support_tier_mapping` data-dependent failure unchanged). +287 new GT3-specific tests vs the 2026-04-11 baseline of 295.
  ...
  - **Remaining Phase 2 plan (~191 h, 3 of 22 units):** **W7.1 (GT3 GarageSetupState + GarageOutputModel, ~24 h, no IBT gating — works against W4.3 GarageRanges + W5.2 CurrentSetup fields) + W8.1 (DB schema migration + per-arch aggregator, ~24 h, parallelizable)** is the next batchable pair (~48 h, disjoint files: `car_model/garage.py` vs `teamdb/models.py` + migration). After this batch, W7.2 (auto-calibrate per-car for GT3, ~56 h) becomes the critical-path next step but is gated on more IBT capture (varied-spring sweeps for BMW M4 GT3 at the same track). Then W8.2 (watcher GT3 CarPath, ~19 h) closes Wave 8; W9.1 + W9.2 (UI/CLI/tests/docs, ~62 h) + W10.1 (E2E smoke + 7 remaining GT3 cars, ~80 h+) close the project. See [`docs/audits/gt3_phase2/SYNTHESIS.md`](docs/audits/gt3_phase2/SYNTHESIS.md) and [`docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md).
