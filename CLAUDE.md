...
- **GT3 Phase 2 — 12 audit PRs merged + Wave 1 + Wave 2 (W2.1, W2.2, W2.3) + Wave 3.1 shipped (2026-04-27):** All 12 audit PRs (#103–#114, ~329 findings, ~511 h estimated) merged into `claude/merge-audits-wave1-DDFyg` (mirrors `gt3-phase0-foundations`). 7 of 22 work units complete (~120 h shipped, ~390 h remaining). The branch carries the full Phase 2 audit corpus under `docs/audits/gt3_phase2/` plus a [`IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) tracking doc updated after every batch. Suite: **543 passed, 32 skipped, 1 deselected, 0 new regressions** (pre-existing `test_support_tier_mapping` data-dependent failure unchanged). +110 new GT3-specific tests vs the 2026-04-11 baseline of 295.
  ...
  - **Wave 2.3 + Wave 3.1 (DONE; commit `c31f3be`, +984/-86, 7 files):**
    ...
    - **Combined state after this batch:** GT3 IBT runs through Step 1 + Step 2 + Step 3 cleanly with real `front_coil_rate_nmm` (from spring range) and real `rear_spring_rate_nmm` (from frequency-isolation). Legal-search now correctly drops heave/third/torsion axes for GT3. Modifier object no longer carries dead heave-floor values. Step 4 still over-targets Porsche 992 RR LLTD and ARB blade encoding wrong (W2.4). Damper polarity wrong for inverted-polarity cars (W3.2). **Setup writer still raises** (W4.1, W4.2).
  - **Remaining Phase 2 plan (~390 h, 15 of 22 units):** W2.4 (ARB/LLTD per-car blade encoding + Porsche 992 RR LLTD target) + W3.2 (damper polarity per-car) is the next batchable pair (~38 h, disjoint files: `arb_solver.py` vs `damper_solver.py`). After that batch, W4.1 (BMW M4 GT3 setup writer) becomes runnable. Critical path: W4.1 → W4.2 (Aston + Porsche 992) → W7.1/W7.2 (auto-calibrate + GarageOutputModel for GT3) → W9.x (UI/CLI/tests/docs) → W10.1 (E2E smoke + 7 remaining GT3 cars) ≈ ~278 h. See [`docs/audits/gt3_phase2/SYNTHESIS.md`](docs/audits/gt3_phase2/SYNTHESIS.md) for the full plan and [`docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) for the rolling status log.
