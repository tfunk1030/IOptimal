...
- **GT3 Phase 2 — 12 audit PRs merged + Wave 1 + Wave 2 + Wave 3 + Wave 4.1 shipped (2026-04-27):** All 12 audit PRs (#103–#114, ~329 findings, ~511 h estimated) merged into `claude/merge-audits-wave1-DDFyg` (mirrors `gt3-phase0-foundations`). 11 of 22 work units complete (~182 h shipped, ~329 h remaining). The branch carries the full Phase 2 audit corpus under `docs/audits/gt3_phase2/` plus a [`IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) tracking doc updated after every batch. Suite: **602 passed, 32 skipped, 1 deselected, 0 new regressions** (pre-existing `test_support_tier_mapping` data-dependent failure unchanged). +169 new GT3-specific tests vs the 2026-04-11 baseline of 295.
  ...
  - **Remaining Phase 2 plan (~329 h, 11 of 22 units):** W4.2 (Aston + Porsche 992 PARAM_IDS, ~24 h, single agent — Porsche has integer ARB encoding, paired rear `TotalToeIn`, `FuelLevel`-in-front section, `FrontBrakesLights`/`ArbSetting`/`DashDisplayPage`; Aston has `FrontBrakesLights`/`FarbBlades`+`RarbBlades`/`AeroBalanceCalculator`/`EpasSetting`/`EnduranceLights`/`NightLedStripColor`/`ThrottleResponse`) is the next critical-path unit. Optional parallel: **W4.2 + W5.2** (analyzer setup_reader GT3 schema dispatch, ~24 h, disjoint files — `output/setup_writer.py` vs `analyzer/setup_reader.py` + `analyzer/setup_schema.py` + `analyzer/sto_adapters.py`). Critical path after that: W4.3 (output guards + iRacing round-trip) → W7.1/W7.2 (auto-calibrate + GarageOutputModel for GT3, gated on more IBT capture) → W9.x (UI/CLI/tests/docs) → W10.1 (E2E smoke + 7 remaining GT3 cars) ≈ ~230 h. See [`docs/audits/gt3_phase2/SYNTHESIS.md`](docs/audits/gt3_phase2/SYNTHESIS.md) for the full plan and [`docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) for the rolling status log.
