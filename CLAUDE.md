...
- **Current calibration status (2026-04-10):**
  - **BMW/Sebring**: `calibrated` (6/6 steps run cleanly, 9 unique setups, garage predictions < 0.09mm). ARB has medium-confidence hand-calibration.
  - **Porsche/Algarve**: `calibrated` (6/6 steps clean, 36 unique setups, garage predictions < 0.07mm on 3 holdout IBTs). Damper zeta from 88 click-sweep sessions, aero compression from 24 sessions, LLTD target = **0.521 from OptimumG physics formula**.
  - **Ferrari/Hockenheim**: `calibrated` (garage predictions < 0.06mm on 4 holdout IBTs, 23 unique setups). 6 contaminated BMW data points removed (2026-04-10). Solver steps blocked (no Hockenheim track profile).
  - **Acura/Hockenheim**: `partial` (8 unique setups, RH < 0.11mm, some deflections limited by rear torsion bar architecture). Solver steps blocked.
