...
- **GT3 Phase 2 — 12 audit PRs merged + Wave 1 + Wave 2 partial shipped (2026-04-27):** All 12 audit PRs (#103–#114, ~329 findings, ~511 h estimated) merged into `claude/merge-audits-wave1-DDFyg` (mirrors `gt3-phase0-foundations`). 5 of 22 work units complete (~96 h shipped, ~415 h remaining). The branch carries the full Phase 2 audit corpus under `docs/audits/gt3_phase2/` plus a new [`IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) tracking doc updated after every batch. Suite: **510 passed, 32 skipped, 1 deselected, 0 new regressions** (pre-existing `test_support_tier_mapping` data-dependent failure unchanged). +77 new GT3-specific tests vs the 2026-04-11 baseline of 295.
  ...
  - **Wave 2 — Solver chain unblocks (PARTIAL; commit `2cbf1e8`, +1152/-134, 12 files):**
    ...
    - **Combined state:** GT3 IBT now runs end-to-end through Step 1 + Step 2 without crashing. **Step 3 still produces non-physical output** (`front_torsion_od_mm=0.0`, `front_rate=0`) because `corner_spring_solver` receives `front_heave_nmm=0` / `rear_third_nmm=0` from the null Step 2 — that's W2.3. **Setup writer still raises** because GT3 PARAM_IDS are empty stubs — that's W4.1 (BMW M4 GT3) and W4.2 (Aston, Porsche 992).
  - **Remaining Phase 2 plan (~415 h, 17 of 22 units):** W2.3 (corner-spring GT3 front-coil branch) + W3.1 (legal_space/modifiers/stint_model `heave_spring=None` guards) are the next batchable pair (~24 h, disjoint files). Then W2.4 (ARB/LLTD per-car blade encoding + Porsche 992 RR LLTD target) + W3.2 (damper polarity per-car) batchable (~38 h). Critical path after that: W4.1 (BMW M4 GT3 setup writer) → W4.2 (Aston + Porsche 992 setup writer) → W7.1/W7.2 (auto-calibrate + GarageOutputModel for GT3) → W9.x (UI/CLI/tests/docs) → W10.1 (E2E smoke + 7 remaining GT3 cars) ≈ ~290 h. See [`docs/audits/gt3_phase2/SYNTHESIS.md`](docs/audits/gt3_phase2/SYNTHESIS.md) for the full plan and [`docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) for the rolling status log.
