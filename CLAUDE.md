...
- **GT3 Phase 2 — 12 audit PRs merged + Wave 1–6 + W7.1 + W8.1 shipped (2026-04-27):** All 12 audit PRs (#103–#114, ~329 findings, ~511 h estimated) merged into `claude/merge-audits-wave1-DDFyg` (mirrors `gt3-phase0-foundations`). **21 of 22 work units complete (~368 h shipped, ~143 h remaining ≈ 72% of total).** Of the 1 unfinished implementation unit (W7.2), one is gated on more IBT capture. Wave 8.2 + Wave 9 + Wave 10 remain. The branch carries the full Phase 2 audit corpus under `docs/audits/gt3_phase2/` plus a [`IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md) tracking doc updated after every batch. Suite: **756 passed, 33 skipped, 1 deselected, 0 new regressions** (pre-existing `test_support_tier_mapping` data-dependent failure unchanged). +323 new GT3-specific tests vs the 2026-04-11 baseline of 295.
  ...
  - **Wave 7.1 + Wave 8.1 (DONE; commit `1d071b8`, +1261/-42, 8 files):**
    ...
    - **Combined state after this batch:** `GarageSetupState` carries GT3 paired-coil + bump-rubber + splitter fields end-to-end. `DirectRegression` can fit on `inv_front_corner_spring` etc. without dropping features. `_setup_key()` distinguishes GT3 IBTs that vary only by front corner-spring (was collapsing). Team server's DB schema + aggregator partition GT3 vs GTP observations. Production DB needs `migrations/0001_gt3_phase2.sql` applied via `psql -f` before next server roll-out.
  - **Remaining Phase 2 plan (~143 h, 1 of 22 units gated on IBT):** **W8.2 (watcher GT3 CarPath detection, ~19 h, no upstream blockers) + W9.1 (webapp + CLI accept GT3, ~30 h)** is the next batchable pair (~49 h, disjoint files: `watcher/{monitor,service}.py` + `desktop/config.py` vs `webapp/` + `__main__.py` + `validation/`). After this batch, W8 + W9.1 close. Critical-path remainder: **W7.2 (auto-calibrate per-car for GT3, ~56 h)** is gated on more IBT capture (varied-spring sweeps for BMW M4 GT3 at the same track). **W9.2 (GT3 regression baselines + docs, ~32 h)** writes the 3 `.sto` baseline fixtures + parameterizes `tests/test_setup_regression.py` + updates `CLAUDE.md` / `skill/per-car-quirks.md` / `docs/calibration_guide.md`. **W10.1 (E2E smoke + 7 remaining GT3 cars, ~80 h+)** is gated on per-car IBT collection. See [`docs/audits/gt3_phase2/SYNTHESIS.md`](docs/audits/gt3_phase2/SYNTHESIS.md) and [`docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md`](docs/audits/gt3_phase2/IMPLEMENTATION_STATUS.md).
